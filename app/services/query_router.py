        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "-- TOP ITEMS BY REVENUE (all years):\n"
                    f"SELECT TOP 10 D.ITEM, D.[DESC], "
                    f"SUM(D.QTYSHIPPED * D.UNITPRICE) AS TotalRevenue, "
                    f"SUM(D.QTYSHIPPED) AS TotalQtySold "
                    f"FROM [{db_name}].[dbo].[OEINVD] D "
                    f"INNER JOIN [{db_name}].[dbo].[OEINVH] H ON D.INVUNIQ = H.INVUNIQ "
                    f"WHERE D.LINETYPE = 1 "
                    f"GROUP BY D.ITEM, D.[DESC] ORDER BY TotalRevenue DESC\n\n"

                    "-- TOP ITEMS BY REVENUE (specific year):\n"
                    f"SELECT TOP 10 D.ITEM, D.[DESC], "
                    f"SUM(D.QTYSHIPPED * D.UNITPRICE) AS TotalRevenue, "
                    f"SUM(D.QTYSHIPPED) AS TotalQtySold "
                    f"FROM [{db_name}].[dbo].[OEINVD] D "
                    f"INNER JOIN [{db_name}].[dbo].[OEINVH] H ON D.INVUNIQ = H.INVUNIQ "
                    f"WHERE H.INVFISCYR = '2023' AND D.LINETYPE = 1 "
                    f"GROUP BY D.ITEM, D.[DESC] ORDER BY TotalRevenue DESC\n\n"

                    "CRITICAL RULES FOR ITEM QUERIES:\n"
                    "1. NEVER join ICITEM using OEINVD.ITEM = ICITEM.ITEMNO — format mismatch! "
                    "   OEINVD.ITEM stores 'A1-450/0' format, ICITEM.ITEMNO stores 'A11030' format. They do NOT match.\n"
                    "   If ICITEM join needed, use: OEINVD.ITEM = ICITEM.FMTITEMNO (not ITEMNO).\n"
                    "2. For item description: use D.[DESC] from OEINVD directly — do NOT rely on ICITEM.[DESC].\n"
                    "3. YEAR FILTER: Always use H.INVFISCYR = 'YYYY' (char). NEVER use ORDDATE/10000.\n"
                    "4. Always filter D.LINETYPE = 1 to exclude misc/freight lines.\n"
                    "5. PROFIT/COST: UNITCOST and EXTICOST are 0 in this DB — cost data not entered. "
                    "   For 'profitable items' queries, use TotalRevenue as proxy and note cost data unavailable.\n\n"
                    ...
                    "-- CUSTOMER OUTSTANDING (overdue > N days, amount > X):\n"
                    f"SELECT ARCUS.NAMECUST, "
                    f"SUM(AROBL.AMTINVCHC) AS TotalOwed, "
                    f"MAX(DATEDIFF(day, CONVERT(date, CAST(AROBL.DATEDUE AS varchar(8))), GETDATE())) AS MaxDaysOverdue "
                    f"FROM [{db_name}].[dbo].[AROBL] AROBL "
                    f"JOIN [{db_name}].[dbo].[ARCUS] ARCUS ON AROBL.IDCUST = ARCUS.IDCUST "
                    f"WHERE AROBL.SWPAID = 0 "
                    f"AND DATEDIFF(day, CONVERT(date, CAST(AROBL.DATEDUE AS varchar(8))), GETDATE()) > 90 "
                    f"GROUP BY ARCUS.IDCUST, ARCUS.NAMECUST "
                    f"HAVING SUM(AROBL.AMTINVCHC) > 10000 "
                    f"ORDER BY TotalOwed DESC\n\n"
                    f"OVERDUE RULES:\n"
                    f"1. Always filter SWPAID = 0 for unpaid invoices only.\n"
                    f"2. Overdue days: DATEDIFF(day, CONVERT(date, CAST(DATEDUE AS varchar(8))), GETDATE()) > N\n"
                    f"3. NEVER use hardcoded dates like DATEDUE < 20220101 for N days overdue.\n"
                    f"4. DaysOverdue in SELECT: use MAX(DATEDIFF(...)) not bare DATEDIFF — avoids GROUP BY error.\n"
                    f"5. Amount threshold goes in HAVING not WHERE.\n\n"
