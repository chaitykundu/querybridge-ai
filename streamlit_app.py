...
if not st.session_state.logged_in:
    ...
    with col:
        ...
        # ══ SIGN IN PAGE ══════════════════════
            ...
            email_si = st.text_input("Email",    placeholder="you@example.com", key="si_email")
            pass_si  = st.text_input("Password", placeholder="••••••••",        key="si_pass", type="password")

            st.markdown("<br>", unsafe_allow_html=True)

            st.markdown('<div class="primary-btn">', unsafe_allow_html=True)
            if st.button("Sign In", key="signin_btn", use_container_width=True):
                if not email_si or not pass_si:
                    st.error("Please enter your email and password.")
                else:
                    ok, msg = do_login(email_si, pass_si)
                    if ok:
                        st.rerun()
                    else:
                        st.error(msg)
